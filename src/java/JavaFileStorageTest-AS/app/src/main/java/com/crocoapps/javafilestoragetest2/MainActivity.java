...
public class MainActivity extends Activity implements JavaFileStorage.FileStorageSetupInitiatorActivity {
...

	{
		...
		protected Void doInBackground(Object... params) {
			...
			try {
				...
				/*
				 * if (e1.displayName == null) throw new
				 * Exception("displayName of "+parentPath+" is null!");
				 * if (e2.displayName.equals(testPath) == false) throw new
				 * Exception("displayName of "+path+" is "+e2.displayName+"!");
				 * 
				 * //try to delete the file to prepare the test. if this fails, we ignore it for
				 * now
				 * try
				 * {
				 * fs.delete(path);
				 ...
				 * throw new
				 * Exception("Either listFiles() didn't throw when listing an unexisting path or the path "
				 * +path+" already exists. Please make sure it doesn't!");
				 * 
				 * Log.d("KP2AJ", "creating folder "+path);
				 * path = fs.createFolder(parentPath, testPath);
				 * Log.d("KP2AJ",
				 * "creating folder returned without exception. Now list its contents.");
				 */
				...
				if (!filesInEmptyDir.isEmpty()) {
					...
					throw new Exception("Received non-empty list with " + filesInEmptyDir.size()
							+ " entries after creating directory!");
				}
				...
				// sleep a second to ensure we have some time between the two modifications (if
				// this is contained in the file version, they should be different)
				Thread.sleep(1000);
				...
				if (version0 != null) {
					if (version0.equals(version1))
						throw new Exception("getCurrentFileVersionFast returned same version string " + version0
								+ " after modification!");
				}

				if (fs.checkForFileChangeFast(filename, version0) == false) {
					// no failure because it's allowed to return false even if there was a change -
					// but it's not good, so warn:
					Log.d("KP2AJ",
							"WARNING! checkForFileChangeFast returned false even though the files were modified!");
				}
		...
		private void checkFileList(String basepath, List<FileEntry> fileList, boolean expectDeletableFile,
				boolean expectDeletableFolder) throws Exception {
				...
				new AsyncTask<Object, Object, Object>() {
					...
					protected Object doInBackground(Object... params) {
						try {
							...
							createStorageToTest(MainActivity.this, MainActivity.this.getApplicationContext(), false)
									.prepareFileUsage(MainActivity.this, path);
							runOnUiThread(new Runnable() {
							...
							runOnUiThread(new Runnable() {
								...
								public void run() {
									Toast.makeText(
											MainActivity.this, "this requires user interaction! "
							...
							runOnUiThread(new Runnable() {
								...
								@Override
								public void run() {
									Toast.makeText(MainActivity.this, t2.getClass().getName() + ": " + t2.getMessage(),
											Toast.LENGTH_LONG).show();
								}
							});
	...
	static JavaFileStorage createStorageToTest(Context ctx, Context appContext, boolean simulateRestart) {
		...
		// storageToTest = new DropboxV2Storage(ctx,"4ybka4p4a1027n6",
		// "1z5lv528un9nre8", !simulateRestart);
		// storageToTest = new DropboxFileStorage(ctx,"4ybka4p4a1027n6",
		// "1z5lv528un9nre8", !simulateRestart);
		// storageToTest = new DropboxAppFolderFileStorage(ctx,"ax0268uydp1ya57",
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if ((requestCode == 1) && (resultCode == RESULT_OK)) {
			...
			// create a new storage to simulate the case that the file name was saved and is
			// used again after restarting the app:
		...
		if ((requestCode == 2124) && (resultCode == RESULT_OK)) {
			...
			Toast.makeText(this, "Selected file path for save: " + path + ". File exists: " + fileExists,
					Toast.LENGTH_LONG).show();
		}
	...
	private void onReceivePathForFileSelect(int requestCode, String path) {
		...
		if (requestCode == 1)
			// new PerformTestTask().execute(path,"TestFileStorage�", storageToTest); //use
			// an umlaut to see how that works
			new PerformTestTask().execute(path, "TestFileStorage", storageToTest);
		else if (requestCode == 2) {
			Intent intent = keepass2android.kp2afilechooser.Kp2aFileChooserBridge.getLaunchFileChooserIntent(this,
					StorageFileProvider.authority, path);
					...
					.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							try {
								...
								onReceivePathForFileSelect(requestCode,
										sftpStorage.buildFullPath(host, port, initialDir, user, pwd));
