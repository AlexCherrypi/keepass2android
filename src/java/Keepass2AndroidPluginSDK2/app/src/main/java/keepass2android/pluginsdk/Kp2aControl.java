...
public class Kp2aControl {
	...
	/**
	 * Creates and returns an intent to launch Keepass2Android for adding an entry
	 * with the given fields.
	 * 
	 * @param fields          Key/Value pairs of the field values. See KeepassDefs
	 *                        for standard keys.
	 ...
	 * @param showUserNotifications if true, the notifications (copy to clipboard,
	 *                              keyboard) are displayed
	 * @param closeAfterOpen        if true, the entry is opened and KP2A is
	 *                              immediately closed
	...
	/**
	 * Creates an intent to query a password entry from KP2A. The credentials are
	 * returned as Activity result.
	 * 
	 * @param searchText Text to search for. Should be a URL or
	...
	/**
	 * Creates an intent to query a password entry from KP2A, matching to the
	 * current app's package .
	 ...
	 * 
	 * @param intent data received in onActivityResult after
	 *               getQueryEntryIntent(ForOwnPackage)
	 * @return HashMap with keys = field names (see KeepassDefs for standard keys)
	 *         and values = values
	 */
