...
public class Strings {
	...
	/**
	 * Action sent from KP2A to the plugin to indicate that the plugin should
	 * request
	...
	/**
	 * Action sent from KP2A to the plugin to indicate that access is not or no
	 * longer valid.
	 */
	public static final String ACTION_REVOKE_ACCESS = "keepass2android.ACTION_REVOKE_ACCESS";

	/**
	 * Action for startActivity(). Opens an activity in the Plugin Host to edit the
	 * plugin settings (i.e. enable it)
	 */
	...
	/**
	 * Action sent from KP2A to the plugin to indicate that an entry output field
	 * was modified/added.
	...
	/**
	 * Action sent from KP2A to the plugin to indicate that an entry activity was
	 * closed.
	 */
	 ...
	 */
	// public static final String EXTRA_ENTRY_DATA =
	// "keepass2android.EXTRA_ENTRY_DATA";

	/**
	 * Json serialized list of fields, transformed using the database context (i.e.
	 * placeholders are replaced already)
	 */
	public static final String EXTRA_ENTRY_OUTPUT_DATA = "keepass2android.EXTRA_ENTRY_OUTPUT_DATA";

	/**
	 * Json serialized lisf of field keys, specifying which field of the
	 * EXTRA_ENTRY_OUTPUT_DATA is protected.
	 */
	...
	/**
	 * Action for an intent from the plugin to KP2A to add menu options regarding
	...
	/**
	 * Used to pass an id for the action. Each actionId may occur only once per
	 * field, otherwise the previous
	...
	/**
	 * Extra for ACTION_ADD_ENTRY_ACTION and ACTION_ENTRY_ACTION_SELECTED to pass
	 * data specifying the action parameters.
	 */
	public static final String EXTRA_ACTION_DATA = "keepass2android.EXTRA_ACTION_DATA";

	/**
	 * Action for an intent from KP2A to the plugin when an action added with
	 * ACTION_ADD_ENTRY_ACTION was selected by the user.
	 * 
	 */
	public static final String ACTION_ENTRY_ACTION_SELECTED = "keepass2android.ACTION_ENTRY_ACTION_SELECTED";

	/**
	 * Extra key for the string which is used to query the credentials. This should
	 * be either a URL for
	 * a web login (google.com or a full URI) or something in the form
	 * "androidapp://com.my.package"
	 */
	...
	/**
	 * Action for an intent from the plugin to KP2A to set (i.e. add or update) a
	 * field in the entry.
	 * May be used to update existing or add new fields at any time while the entry
	 * is opened.
	 */
	public static final String ACTION_SET_ENTRY_FIELD = "keepass2android.ACTION_SET_ENTRY_FIELD";

	/**
	 * Actions for an intent from KP2A to the plugin to inform that a database was
	 * opened, closed, quicklocked or quickunlocked.
	 */
	...
	/**
	 * Extra for ACTION_OPEN_DATABASE and ACTION_CLOSE_DATABASE containing a
	 * filepath which is used
	 * by KP2A internally to identify the file. Use only where necessary, might
	 * contain credentials
