...
public class DecoratedHostnameVerifier implements HostnameVerifier {
    ...
    public DecoratedHostnameVerifier(HostnameVerifier hostnameVerifier,
            ICertificateErrorHandler mCertificateErrorHandler) {
    ...
    public boolean verify(String host, SSLSession sslSession) {
        ...
        if ((certificateErrorHandler == null)
                || (!certificateErrorHandler.onValidationError("Failed to verify host " + host))) {
            // certificate error handler does not allow to ignore the error
            return false;
        }
        // certificate error handler did display a warning probably and allowed to
        // ignore the error.
        return true;
