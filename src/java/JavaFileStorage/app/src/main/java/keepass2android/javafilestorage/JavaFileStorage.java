...
public interface JavaFileStorage {
	...
	// public void onActivityResult(Activity activity, final int requestCode, final
	...
	// prepare the file usage. if not possible, throw an exception. Must throw
	// UserInteractionRequiredException if the
	// problem can be resolved by the user. Caller should then retry with
	// prepareFileUsage(FileStorageSetupInitiatorActivity activity, String path, int
	// requestCode, boolean alwaysReturnSuccess)
	public void prepareFileUsage(Context appContext, String path) throws UserInteractionRequiredException, Throwable;

	// prepare the file usage. if necessary, use the activity to interact with the
	// user, e.g. to grant access.
	...
	// returns something like "myfile.txt" from the given path, i.e. it's
	// displayable and only the last part of the path
	...
	// creates a folder "newDirName" in parentPath and returns the path of the new
	// folder
	public String createFolder(String parentPath, String newDirName) throws Exception;

	// returns the path of a file "newFileName" in the folder "parentPath"
	// this may create the file if this is required to get a path (if a UUID is part
	// of the file path)
	...
	public void onRequestPermissionsResult(FileStorageSetupActivity activity, int requestCode, String[] permissions,
			int[] grantResults);
