...
public abstract class JavaFileStorageBase implements JavaFileStorage {
	...
	protected String decode(String encodedString)
			throws UnsupportedEncodingException {
		// the first version of encode/decode used ISO 8859-1 which doesn't work with
		// Cyrillic characters
		// this is why we need to check for the prefix, even though all new strings are
		// UTF8 encoded.
	...
	protected void finishWithError(final FileStorageSetupActivity setupAct, Exception error) {
		...
		// This can make sense if there is a higher-level FileStorage which has the file
		// cached.
