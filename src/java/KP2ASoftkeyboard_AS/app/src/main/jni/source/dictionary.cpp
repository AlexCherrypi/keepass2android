...
namespace latinime
{
                                   ...
                                   int *nextLetters, int nextLettersSize)
    {
        ...
        // Get the word count
        suggWords = 0;
        while (suggWords < mMaxWords && mFrequencies[suggWords] > 0)
            suggWords++;
        if (DEBUG_DICT)
            LOGI("Returning %d words", suggWords);
    ...
    Dictionary::getAddress(int *pos)
    {
        ...
        if (address < 0 || address >= mDictSize)
            return 0;
        return address;
    }
    ...
    Dictionary::addWord(unsigned short *word, int length, int frequency)
    {
        ...
        {
            if (frequency > mFrequencies[insertAt] || (mFrequencies[insertAt] == frequency && length < wideStrLen(mOutputChars + insertAt * mMaxWordLength)))
            {
                break;
            }
            insertAt++;
    ...
    Dictionary::addWordBigram(unsigned short *word, int length, int frequency)
    {
        ...
        {
            if (frequency > mBigramFreq[insertAt] || (mBigramFreq[insertAt] == frequency && length < wideStrLen(mBigramChars + insertAt * mMaxWordLength)))
            {
                break;
            }
            insertAt++;
    ...
    Dictionary::searchForTerminalNode(int addressLookingFor, int frequency)
    {
        ...
        while (!found)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else if (getFirstBitOfByte(&pos))
                { // terminal
                    if (addressLookingFor == (pos - 1))
                    { // found !!
                        depth++;
